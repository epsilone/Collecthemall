...
class t_html_generator : public t_generator {
 public:
    ...
    : t_generator(program)
  {
    ...
    iter = parsed_options.find("ajax_calls");
    ajax_calls_ = (iter != parsed_options.end());
  ...
  void generate_ajax(t_service* tservice);
...
void t_html_generator::generate_program() {
  ...
  // PN: added support for ajax_calls
  if (ajax_calls_) {
      f_out_ << "<link href=\"css/jquery-ui.css\" rel=\"stylesheet\" type=\"text/css\" />" <<endl;
      f_out_ << "<link href=\"css/thrift_service.css\" rel=\"stylesheet\" type=\"text/css\" />" <<endl;
	  ...
	  if (!services.empty()) {
		  if (services.size() == 1) {
			  ...
			  throw "compiler error: ajax_calls only support one service per module!";
		  }
	  }
  }
  ...
  if (!program_->get_services().empty()) {
	...
	// PN: added support for ajax_calls
	if (ajax_calls_) {
		f_out_ << "<div id=\"service_info\">" << endl;
		f_out_ << "<form>" << endl;
		f_out_ << "<label>service url</label><input type=\"text\" id=\"service_url\" />" << endl;
		f_out_ << "<label>headers</label><input type=\"text\" id=\"service_headers\" />" << endl;
		f_out_ << "</form>" << endl;
	}
    // Generate services
...
void t_html_generator::generate_service(t_service* tservice) {
  ...
  for ( ; fn_iter != functions.end(); fn_iter++) {
	...
	// PN: added support for ajax_calls
	if (ajax_calls_) {
		f_out_ << "<div id=\"" << fn_name << "_accordion\">" << endl;
		f_out_ << "<h3>Definition</h3>" << endl;
		f_out_ << "<div>" << endl;
	}
	...
	if (ajax_calls_) {
		f_out_ << "</div>" << endl;
		f_out_ << "<h3>Try it</h3>" << endl;
		f_out_ << "<div>" << endl;
		f_out_ << "<form id=\"" << fn_name << "\">" << endl;
		vector<t_field*> args = (*fn_iter)->get_arglist()->get_members();
		vector<t_field*>::iterator arg_iter = args.begin();
		for ( ; arg_iter != args.end(); arg_iter++) {
			f_out_ << "<label>" << (*arg_iter)->get_name() << ":</label><input type=\"text\" id=\"" << fn_name << "_" << (*arg_iter)->get_name() << "\" />" << endl;
		}
		f_out_ << "</form>" << endl;
		f_out_ << "<button id=\"" << fn_name << "_do\">submit</button>" << endl;
		f_out_ << "<div id=\"response\">" << endl;
		f_out_ << "<div name=\"server_response\" id=\"" << fn_name  << "_response\" class=\"ui-widget-content ui-corner-all\">" << endl;
		f_out_ << "</div>" << endl;
		f_out_ << "</div>" << endl;
		f_out_ << "</div>" << endl;
		f_out_ << "</div>" << endl;
...
void t_html_generator::generate_ajax(t_service* service) {
	...
	f_out_ << "$('div[name=\"server_response\"]').hide();" << endl;
	f_out_ << "var on_success = function(selector, result) {" << endl;
	f_out_ << "var response_content = \"\";" << endl;
    f_out_ << "for (var property in result) {" << endl;
    f_out_ << "if (typeof(result[property]) != \"function\") {" << endl;
    f_out_ << "response_content += property;" << endl;
    f_out_ << "response_content += \": \";" << endl;
    f_out_ << "response_content += result[property];" << endl;
    f_out_ << "response_content += \"<br />\";" << endl;
    f_out_ << "}" << endl;
	f_out_ << "}" << endl;
	f_out_ << "write_response(selector, response_content);" << endl;
	f_out_ << "}" << endl;
	f_out_ << "var on_error = function(selector, exception) {" << endl;
	f_out_ << "console.log(exception);" << endl;
	f_out_ << "write_response(selector, \"<p>\" + exception + \"</p>\");" << endl;
	f_out_ << "}" << endl;
    f_out_ << "var write_response = function(selector, response_content) {" << endl;
	f_out_ << "$(selector).html('<h5 class=\"response_header  ui-widget-header ui-corner-all\">Response</h5><p>' + response_content +'</p>')" << endl;
	f_out_ << "$(selector).show();" << endl;
	f_out_ << "}" << endl;
	...
	for ( ; fn_iter != functions.end(); fn_iter++) {
		...
		f_out_ << "$(\"#" << fn_name << "_accordion\").accordion({" << endl;
		f_out_ << "collapsible: true," << endl;
		f_out_ << "heightStyle: \"content\"" << endl;
		f_out_ << "});" << endl;
		f_out_ << endl;
		f_out_ << "$(\"#" << fn_name << "_do\").click(function(event) {" << endl;
        f_out_ << "event.preventDefault();" << endl;
		f_out_ << "var transport = new Thrift.Transport($(\"#service_url\").val());" << endl;
		f_out_ << "var protocol = new Thrift.Protocol(transport);" << endl;
		f_out_ << "var client = new " << service->get_name() << "Client(protocol);" << endl;
        f_out_ << "try {" << endl;
        f_out_ << "var result = client." << fn_name << "(";
		vector<t_field*> args = (*fn_iter)->get_arglist()->get_members();
		vector<t_field*>::iterator arg_iter = args.begin();
		while (arg_iter != args.end()) {
			const t_type* field_type = (*arg_iter)->get_type();
			if (field_type->is_base_type()) {
				t_base_type::t_base base_type = ((t_base_type*)field_type)->get_base();
				switch (base_type) {
				...
				case t_base_type::TYPE_DOUBLE:
					f_out_ << "Number($(\"#" << fn_name << "_" << (*arg_iter)->get_name() << "\").val())";
					break;
				case t_base_type::TYPE_BOOL:
					f_out_ << "Boolean(Number($(\"#" << fn_name << "_" << (*arg_iter)->get_name() << "\").val()))";
					break;
				case t_base_type::TYPE_STRING:
					f_out_ << "$(\"#" << fn_name << "_" << (*arg_iter)->get_name() << "\").val()";
					break;
				}
			}
			...
			else if (field_type->is_list() || field_type->is_set()) {
				f_out_ << "$(\"#" << fn_name << "_" << (*arg_iter)->get_name() << "\").val().split(',')";
			}
			else if (field_type->is_map()) {
				throw "compiler error: ajax_calls map support is not yet implemented!";
			}
			else {
				// Default: try to send it as a string
				f_out_ << "$(\"#" << fn_name << "_" << (*arg_iter)->get_name() << "\").val()";
			}
			++arg_iter;
			if (arg_iter != args.end()) {
		...
		f_out_ << ");" << endl;
        f_out_ << "on_success(\"#" << fn_name << "_response\", result);" << endl;
        f_out_ << "}" << endl;
        f_out_ << "catch (exception) {" << endl;
        f_out_ << "on_error(\"#" << fn_name << "_response\", exception);" << endl;
        f_out_ << "}" << endl;
		f_out_ << "});" << endl;
	}
...
"    ajax_calls:           Generate ajax calls for each service.\n"
)
